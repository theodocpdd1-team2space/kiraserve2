...
const coordinatorNavItems = [
  { title: "Dashboard", href: "dashboard", icon: Home },
  { title: "My Divisions", href: "divisions", icon: Network },
  { title: "Scheduling", href: "scheduling", icon: CalendarCheck },
];

const memberPortalNavItems = [
  { title: "My Portal", href: "me", icon: UserRound },
  { title: "My Schedule", href: "me/schedule", icon: CalendarCheck },
];
...
export default async function ChurchAppShell({
  ...
  const navItems =
    access.canManageChurch
      ? ownerAdminNavItems
      : access.isCoordinator
        ? coordinatorNavItems
        : memberPortalNavItems;

  const showBillingCard = access.canViewBilling || access.canManageChurch;
    ...
    <main className="min-h-screen bg-[#F6F7F1] text-[#0B0D0F] selection:bg-[#D4F93A] selection:text-black">
      <div className="flex min-h-screen">
        <aside className="fixed inset-y-0 left-0 z-50 hidden w-[284px] border-r border-black/[0.06] bg-white p-5 lg:flex lg:flex-col">
          ...
          >
            <div className="flex h-12 w-12 items-center justify-center rounded-[18px] bg-[#D4F93A] text-black shadow-sm">
              <span className="text-2xl font-black tracking-[-0.12em]">K</span>
            </div>
          ...
          <div className="mb-5 rounded-[22px] border border-black/10 bg-[#FAFAFA] p-4">
            ...
            {access.isCoordinator && !access.canManageChurch && (
              <p className="mt-1 text-xs font-medium text-black/45">
                {access.coordinatedDivisionIds.length} coordinated division
              </p>
            )}
          </div>
          ...
          <nav className="space-y-1.5">
            {navItems.map((item) => {
              const isActive =
                active === item.href || active === item.href.split("/")[0];
        ...
        <nav className="fixed bottom-5 left-1/2 z-50 flex w-[90%] max-w-[420px] -translate-x-1/2 items-center justify-between rounded-[32px] bg-[#070A0D]/95 px-4 py-3 shadow-[0_22px_55px_-18px_rgba(0,0,0,0.7)] backdrop-blur-xl lg:hidden">
          <MobileNavIcon
            href={`/church/${church.slug}/${navItems[0]?.href || "dashboard"}`}
            active={active === navItems[0]?.href}
            icon={navItems[0]?.icon || Home}
          />

          <MobileNavIcon
            href={`/church/${church.slug}/${navItems[1]?.href || "divisions"}`}
            active={active === navItems[1]?.href}
            icon={navItems[1]?.icon || Network}
          />

          <Link
            href={
              access.canManageChurch || access.isCoordinator
                ? `/church/${church.slug}/scheduling`
                : `/church/${church.slug}/me/schedule`
            }
          ...
          <MobileNavIcon
            href={
              access.canManageChurch
                ? `/church/${church.slug}/settings`
                : `/church/${church.slug}/me`
            }
            active={active === "settings" || active === "me"}
            icon={MoreHorizontal}
          />
        </nav>
      </div>
